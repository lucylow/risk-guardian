...
const FEATURES = [
  { icon: "🥪", title: "Sandwich Attack Detection", desc: "Real-time mempool scanning flags front-running bot patterns before they strike your swap.", badge: "AI-Powered", badgeCls: "bg-primary/10 border-primary/20 text-primary" },
  { icon: "💧", title: "Liquidity Pool Health", desc: "Deep analysis of TVL, LP concentration, and 24h volume forecasts pool stability using OnePredict data.", badge: "Predictive", badgeCls: "bg-risk-safe/10 border-risk-safe/20 text-risk-safe" },
  { icon: "🪪", title: "Wallet Reputation Scoring", desc: "Leverages OneID to flag risky addresses, anomalous on-chain behavior, and known exploiter wallets.", badge: "OneID", badgeCls: "bg-accent/10 border-accent/20 text-accent" },
  { icon: "🛡️", title: "Auto-Protect Mode", desc: "Automatically routes high-risk swaps through a private mempool to neutralize MEV exposure.", badge: "Automated", badgeCls: "bg-risk-moderate/10 border-risk-moderate/20 text-risk-moderate" },
  { icon: "🔗", title: "Native OneChain Integration", desc: "Built on OneDEX, OnePredict, OneID, and OneWallet APIs — deep ecosystem coverage from day one.", badge: "Ecosystem", badgeCls: "bg-primary/10 border-primary/20 text-primary" },
];
    ...
    <div className="max-w-3xl mx-auto">
      ...
      <div className="glass-card rounded-2xl p-8 mb-8 border-l-4 border-primary">
        <div className="flex items-start gap-4">
          <div className="w-10 h-10 rounded-xl bg-primary/10 border border-primary/20 flex items-center justify-center text-lg shrink-0 mt-0.5">🎯</div>
          <div>
      ...
      <div className="mb-10">
        ...
        <div className="space-y-3">
          {FEATURES.map((f) => (
            <div key={f.title} className="glass-card rounded-xl p-5 flex items-start gap-4 hover:border-primary/30 transition-colors">
              <div className="w-10 h-10 rounded-xl bg-surface flex items-center justify-center text-xl shrink-0">{f.icon}</div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 flex-wrap mb-1">
                  <p className="font-display font-semibold text-foreground">{f.title}</p>
                  <span className={`text-[10px] font-mono font-bold px-2 py-0.5 rounded-full border ${f.badgeCls}`}>{f.badge}</span>
                </div>
      ...
      <div className="mb-10">
        ...
        <div className="glass-card rounded-2xl overflow-hidden">
          {STACK.map((s, i) => (
            <div key={s.label} className={`flex justify-between items-center px-6 py-4 hover:bg-surface-highlight/50 transition-colors ${i < STACK.length - 1 ? "border-b border-border" : ""}`}>
      ...
      <div className="mb-10">
        ...
        <div className="space-y-3">
          {TEAM.map((t) => (
            <div key={t.name} className="glass-card rounded-xl p-5 flex items-center gap-4 hover:border-primary/30 transition-colors">
              <div className="w-12 h-12 rounded-full bg-primary/15 border border-primary/20 flex items-center justify-center text-2xl">{t.emoji}</div>
              <div>
      ...
      <div className="text-center glass-card rounded-2xl p-10 relative overflow-hidden">
        ...
        <div className="relative">
          ...
          <p className="font-display font-bold text-2xl mb-2 text-foreground">Ready to swap safely?</p>
          <p className="text-foreground-muted mb-6 text-sm">Try the live demo and see your Safety Score in action.</p>
          <a href="/#demo" className="btn-primary inline-flex items-center gap-2 px-7 py-3 rounded-xl font-display font-semibold">
